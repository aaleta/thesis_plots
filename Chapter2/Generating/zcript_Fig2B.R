...
singlePlot = function(data,color,title,fit)
{
	...
	p = ggplot(data,aes(x=x,y=y,fill=color)) +
		...
		ggtitle(title)+
		scale_y_continuous(expand=c(0,0),limits=c(0,.25),breaks=seq(0,.25,by=.05))  +
		scale_x_continuous(expand=c(0,0),breaks=seq(2.5,48.5,by=4),labels=c(paste(seq(1,41,by=4),seq(4,44,by=4),sep="-"),"45+"),limits=c(0,50))+
		annotate("text",x=28,y=0.2,label=paste0("mu==","'",sprintf("%.2f",round(fit$mu,2)),"'","~~theta==","'",sprintf("%.2f",round(fit$size,2)),"'"),parse=T,size=12)
}
...
plotMatrix = function()
{
	m = read.table("Data/matrix_data.txt")
	data = data.frame(x = c(1,2,1,2), y = c(2,2,1,1), values = c(m[1,1],m[1,2],m[2,1],m[2,2]))

	p = ggplot(data, aes(x = x, y = y)) +
		theme_thesis() +
		geom_raster(aes(fill = values)) +
		geom_text(aes(label = round(values,2)), size = 10) +
		...
		scale_x_continuous(expand = c(0,0), breaks = c(1,2), labels = c("C","A")) +
		scale_y_continuous(expand = c(0,0), breaks = c(1,2), labels = c("A","C")) +
		coord_fixed()

	return(p)
}

bothPlots = function(print.plot=T)
{
	data = buildFitDF(countBracket(read.table("Data/ALL.txt")[,1]))
	fit = fitDF(data)
	data = reshapeDF(data)
	p1 = ggplotGrob(singlePlot(data,"#99751B","P(k)",fit))

	p2 = ggplotGrob(plotMatrix())

	p = plot_grid(p1,plot_grid(NULL,p2,NULL,nrow=3,rel_heights=c(1,4,1)),ncol=2,rel_widths=c(1.5,1))
	if(print.plot){
		pdf("Plots/Fig2B.pdf",width=12,height=6)
		print(p)
		dev.off()
	}
	return(p)
