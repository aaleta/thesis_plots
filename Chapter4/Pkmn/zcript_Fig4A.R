...
{
	load("Data/rectangles_d6.RData")
	load("Data/lines_d6.RData")
	load("Data/circle_d6.RData")
	...
	p = p +	geom_line(data=circle,aes(x=x,y=y,group=factor(y)),color="black") +
		...
		scale_x_continuous(expand=c(0,0),limits=c(1,4*M),breaks=c(1,M/2,M,1.5*M,2*M,2.5*M,3*M,3.5*M),labels=c("06d00h","06d12h","06d00h","06d12h","06d00h","06d12h","06d00h","06d12h")) +
		annotate("text",x=16000,y=0.13,label="0.10") +
		annotate("text",x=6000,y=0.32,label="0.30") +
		annotate("text",x=4000,y=0.52,label="0.50") +
		annotate("text",x=3000,y=0.70,label="0.70",vjust=-0.5)

	if(print.plot){
		cairo_pdf(paste0("Plots/Fig4A.pdf"),width=8,height=8)
