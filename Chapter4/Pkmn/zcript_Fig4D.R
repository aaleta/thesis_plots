...
plotD = function(print.plot=F)
{
	load("Data/data_d8.RData")
	load("Data/rectangles2_d8.RData")
	load("Data/votes_d8.RData")
	...
	p = p +	geom_hline(yintercept=0.8,linetype=2) +
		...
		scale_y_continuous(limits=c(0,1),expand=c(0,0),breaks=c(0,0.25,0.50,0.80,1)) +
	...
	if(print.plot){
		cairo_pdf(paste0("Plots/Fig4D.pdf"),width=12,height=6)
		plot(p)
		dev.off()
	}
