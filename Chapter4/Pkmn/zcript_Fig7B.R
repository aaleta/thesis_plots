...
plotB = function(print.plot=F)
{
	...
	load("Data/participation_2a.RData")
	load("Data/participation_2b.RData")
	...
	p = p +	geom_hline(yintercept=0.75,linetype=2) +
		...
		geom_point(data=a,aes(x=x,y=democracy,color="1 participation"),size=1.5) +
		geom_point(data=b,aes(x=x,y=democracy,color="Several participations"),size=1.5) +
		...
		scale_color_manual(name="",values=c("#B24745","#00A1D5"),labels=c("Visitors","Commited players")) +
 		guides(linetype = guide_legend(override.aes = list(size=2)),colour = guide_legend(override.aes = list(size=6))) +
	...
	if(print.plot){
		cairo_pdf(paste0("Plots/Fig7B.pdf"),width=14,height=6)
		plot(p)
		dev.off()
	}
