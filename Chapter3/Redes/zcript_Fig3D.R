...
plotDSF = function(print.plot=F)
{
	#UDU
	a = read.table("Data/UDU/diagramC_g2.7_mu0.10_p0.5_SF.txt",header=T)
	...
	#DDD
	a = read.table("Data/DDD/diagramC_g2.7_mu0.10_p0.5_SF.txt",header=T)
	...
	if(print.plot){
		pdf(paste0("Plots/Fig3D.pdf"),width=8,height=6)
		plot(p)
		dev.off()
	}
